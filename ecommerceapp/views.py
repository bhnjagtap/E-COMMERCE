...
def checkout(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, f"Error in placing order: {e}")
    ...
    if verify:
        if response_dict['RESPCODE'] == '01':
            ...
            try:
                ...
                order.oid = response_dict['ORDERID']
...
def profile(request):
    ...
    for order in items:
        try:
            order_id = order.oid.replace("ShopyCart", "")
            status = OrderUpdate.objects.filter(order_id=int(order_id))
            order_statuses.extend(status)
        except ValueError:
            continue

    context = {"items": items, "statuses": order_statuses}
    return render(request, "profile.html", context)
